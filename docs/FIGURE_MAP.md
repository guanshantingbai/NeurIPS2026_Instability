...
**Minimal full-path (not paper 81-setting grid):** On a machine with MVTec at `~/datasets/mvtec`, **`docs/FULLPATH_PROMPTAD.md`** section **(C)** documents a **two-seed** `train,infer,export` smoke (`mvtec` / `bottle` / `k=1` / seeds `111,222`) into `outputs/promptad_fullpath_minimal/`, then unified raw + Sec 3 / Sec 4 pairwise scripts — **`minimal full-path verified`** in **`docs/END_TO_END_VALIDATION.md`** §11.
