...
def get_dir_from_args(TASK, root_dir, **kwargs):
    ...
    # Checkpoints always live under ``root_dir`` (training / weight location).
    # Optional ``output_root`` (test_cls only): write csv + imgs elsewhere so inference
    # artifacts do not mix with a separate retraining output tree.
    out = kwargs.get("output_root")
    if out is None or (isinstance(out, str) and not str(out).strip()):
        artifact_root = root_dir
    else:
        artifact_root = str(out).strip()

    csv_dir = os.path.join(artifact_root, f'{dataset}', f'k_{k_shot}', 'csv')
    check_dir = os.path.join(root_dir, f'{dataset}', f'k_{k_shot}', 'checkpoint')
    img_dir = os.path.join(artifact_root, f'{dataset}', f'k_{k_shot}', 'imgs')
